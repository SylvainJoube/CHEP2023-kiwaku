...
acts_covfie_list          = load_file("data2/slice_covfie_blop-debian11_step1000.txt")
...
acts_kiwaku_inline_list   = load_file("data2/slice_kiwaku_inline_blop-debian11_step1000.000000.txt")
acts_kiwaku_noinline_list = load_file("data2/slice_kiwaku_noinline_blop-debian11_step1000.000000.txt")
acts_standalone_list      = load_file("data2/slice_standalone_blop-debian11_step1000.000000.txt")

# acts_standalone_list_v3   = load_file("bench_acts_field_standalone_cassidi_v3.txt")
...
check_same_results(acts_standalone_list, acts_covfie_list)
# check_same_results(acts_standalone_list, acts_standalone_list_v3)


# acts_covfie_list 
# acts_kiwaku_inline_list 
# acts_kiwaku_noinline_list 
# acts_standalone_list 

# ldiff0 = make_diff_list(acts_kiwaku_inline_list, acts_kiwaku_inline_list)
# ldiff1 = make_diff_list(acts_kiwaku_inline_list, acts_covfie_list)
# ldiff2 = make_diff_list(acts_kiwaku_inline_list, acts_kiwaku_noinline_list)
# ldiff3 = make_diff_list(acts_kiwaku_inline_list, acts_standalone_list)
...
ldiff3 = make_diff_list(acts_standalone_list, acts_covfie_list)
# ldiff4 = make_diff_list(acts_standalone_list, acts_standalone_list_v3)
...
ptotal_1D   = make_1D_list_every_auto(acts_covfie_list, "z_value")

# range(1, len(ldiff)+1) #make_1D_list_every(lorentz_covfie_list, "ptotal", 3)

plt.plot  (range(1, len(ldiff0)+1), ldiff0, color="grey", label="standalone v1", linestyle="dashdot", linewidth=line_width)
plt.plot  (range(1, len(ldiff1)+1), ldiff1, color="blue", label="kiwaku inline", linestyle="dashed", linewidth=line_width)
plt.plot  (range(1, len(ldiff2)+1), ldiff2, color="maroon", label="kiwaku not inline", linestyle="dotted", linewidth=line_width)
plt.plot  (range(1, len(ldiff3)+1), ldiff3, color="green", label="covfie", linestyle="solid", linewidth=line_width)
