...
struct acts_data_t
{
  ...
  inline pt3D<float> at(float p0, float p1, float p2)
  {
      ...
      return at_affine2(p0, p1, p2);
    #else
      return at_affine(p0, p1, p2);
    #endif
  }
  
  inline pt3D<float> at_affine(float p0, float p1, float p2);
  inline pt3D<float> at_affine2(float p0, float p1, float p2);
...
void acts_data_t::read_acts_file()
{
  ...
  data.resize(read_numel);
  ifs.read(reinterpret_cast<char *>(data.data()), read_numel * sizeof(pt3D<float>));
...
inline pt3D<float> acts_data_t::at_affine2(float p0, float p1, float p2)
{
  std::array<float, 3> kv{p0, p1, p2};

  auto kres = m_transform_optimized * kv; // m_transform

  // ==== LINEAR ====
  return acts_data_t::at_linear(kres[0], kres[1], kres[2]);
  // return acts_data_t::at_linear(100UL,100UL,150UL);
}

inline pt3D<float> acts_data_t::at_affine(float p0, float p1, float p2)
{
  
  // std::cout << "p = acts.at_affine(" << p0 << ", " << p1 << ", " << p2 << "); s += p[0] + p[1] + p[2]; \n";
...
{
  // std::cout << "p = acts.at_strided" << c << "; s += p[0] + p[1] + p[2]; \n";
  ...
  auto value = at_array(strided_to_linear(c)); // idx
...
{ 
  ...
  #if SMEM_ENABLE_LOG
    // void* cptr = static_cast<void*>(&data[c]);
    // void* bptr = static_cast<void*>(&data[0]);
    std::size_t d = std::distance(&data[0], &data[c]);
    mem_test_write_f << d << "\n"; // std::size_t(cptr) - std::size_t(bptr)
  #endif
