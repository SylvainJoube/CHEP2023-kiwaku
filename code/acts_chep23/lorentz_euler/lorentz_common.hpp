...
struct Lorentz
{
  ...
  particles_creation(std::size_t particles, std::size_t imom)
  {
    ...
    for (std::size_t i = 0; i < particles; ++i) {
      ...
      objs[i].mom[0] =
          static_cast<float>(imom) * std::sin(theta) * std::cos(phi);
      objs[i].mom[1] =
          static_cast<float>(imom) * std::sin(theta) * std::sin(phi);
      objs[i].mom[2] = static_cast<float>(imom) * std::cos(theta);
    }
    return objs;
  }
...
struct lorentz_bench_t
{
  ...
  static void
  execute_custom(std::size_t particles, std::size_t steps, std::size_t imom) // , FIELD_T* field
  {
    ...
    for (uint i = 0; i < iterations; ++i)
    {
      ...
      res = Lorentz<Euler, Deep>::iteration<FIELD_T>(particles, steps, imom, os_table);
